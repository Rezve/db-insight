...
export default function TableSchema({ tableName }: TableSchemaProps) {
  ...
  function buildFullDDL() {
    const parts = [ddl.createTable(schemaName, tableOnly, columns, extended)];
    const indexDDL = ddl.indexes(schemaName, tableOnly, indexes);
  ...
  const ddlText = view === "sql" ? buildFullDDL() : "";
        ...
        <CardContent className="p-0">
          <div className="relative">
            <pre
              ...
              // Safe: ddlText is generated by the driver DDL builders, not from user input.
              // highlightDDL escapes &, <, > before injecting HTML.
              dangerouslySetInnerHTML={{ __html: highlightDDL(ddlText) }}
            />
            <Button
              size="sm"
              variant="secondary"
